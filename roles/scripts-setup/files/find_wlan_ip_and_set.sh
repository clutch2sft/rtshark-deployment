...
check_wlan0_agclogen() {
    while true; do
        ...
        if [ "$connection" == "agclogen" ]; then
            log_message "find_wlan_ip_and_set" "INFO" "wlan0 is connected to agclogen."
            break
        elif [ -n "$connection" ]; then
            log_message "find_wlan_ip_and_set" "ERROR" "wlan0 is connected to a different network: $connection. Exiting script."
            exit 1
        else
            log_message "find_wlan_ip_and_set" "DEBUG" "Waiting for wlan0 to connect to agclogen..."
            sleep 5
        fi
    done
}
...
for ip in $(seq 254 -1 247); do
    if arping -0 -c 2 -w 3 -I wlan0 120.13.212.$ip | grep -q "Timeout"; then
        log_message "find_wlan_ip_and_set" "INFO" "Available IP: 120.13.212.$ip"
        available_ip="120.13.212.$ip"
        available_gw="120.13.212.1"
        break
    else
        log_message "find_wlan_ip_and_set" "DEBUG" "IP 120.13.212.$ip is in use. Checking next."
    fi
done

# Check if an available IP was found
if [ -z "$available_ip" ]; then
    log_message "find_wlan_ip_and_set" "WARN" "No available 120.13.212.x addresses in the specified range. Exiting."
    exit 1
else
    log_message "find_wlan_ip_and_set" "INFO" "Assigning new IP:$available_ip and gateway:$available_gw to agclogen"
